...
Criar a pasta dados-brutos para receber os arquivos a serem baixados e descomprimidos. Como esses arquivos são grandes e públicos, eles não são disponibilizados no repositório. Ao invés disso, a programação cria a pasta e baixa os arquivos dos repositórios institucionais na internet.
...
Download e descompressão dos microdados da Pesquisa de Orçamentos Familiares 2017-2018.

```{r}
url <- "https://ftp.ibge.gov.br/Orcamentos_Familiares/Pesquisa_de_Orcamentos_Familiares_2017_2018/Microdados/Dados_20230713.zip"

download.file(url, "data-raw/pof.zip")
unzip("data-raw/pof.zip", "DESPESA_INDIVIDUAL.txt", exdir = "data-raw")
```

Leitura do arquivo descomprimido da POF. A execução do comando de leitura do arquivo (read.fwf) é demorada, podendo durar vários minutos em alguns computadores. Desta forma, o resultado desse processo de descompressão será salvo no formato RDS para facilitar novas tentativas de rodar o restante do código.

```{r}
C.POF <- read.fwf("data-raw/DESPESA_INDIVIDUAL.txt",
                  widths = c(2,4,1,9,2,1,2,2,2,7,2,10,2,2,1,1,1,12,10,1,2,14,14,10),
                  na.strings=c(" "),
                  col.names = c("UF", "ESTRATO_POF", "TIPO_SITUACAO_REG", 
                                "COD_UPA", "NUM_DOM", "NUM_UC", "COD_INFORMANTE", 
                                "QUADRO", "SEQ", "V9001", "V9002", "V8000", "V9010",                                 "V9011", "V9012", "V4104", "V4105", "DEFLATOR", 
                                "V8000_DEFLA", "COD_IMPUT_VALOR", 
                                "FATOR_ANUALIZACAO", "PESO", "PESO_FINAL",
                                "RENDA_TOTAL"),
                  dec=".")
head(C.POF)
saveRDS(C.POF, "rds/POF.rds")
```

## Tabela de Recursos e Usos

Download e descompressão da Tabela de Recursos e Usos de 2018 do IBGE.

```{r}
url <- "https://ftp.ibge.gov.br/Contas_Nacionais/Sistema_de_Contas_Nacionais/2021/tabelas_xls/tabelas_de_recursos_e_usos/nivel_68_2010_2021_xls.zip"

download.file(url, "data-raw/tru.zip")
unzip("data-raw/tru.zip", "68_tab2_2018.xls", exdir = "data-raw")
```

Importação dos dados de consumo intermediário da TRU2.

```{r}
CI.TRU2 <- read_xls("data-raw/68_tab2_2018.xls", sheet = "CI",
                    skip = 3, .name_repair = "unique_quiet")
CI.ij <- CI.TRU2[2:129, 3:70]
CI.j <- CI.TRU2[131, 3:70]
CI.i <- CI.TRU2[2:129, 71]
CI <- CI.TRU2[131, 71]
rm(CI.TRU2)
```

Importação dos dados de demmanda final da TRU2.

```{r}
DF.TRU2 <- read_xls("data-raw/68_tab2_2018.xls", sheet = "demanda",
                    skip = 3, .name_repair = "unique_quiet")
C.i <- DF.TRU2[2:129, 6]
X.i <- DF.TRU2[2:129, 3]
C <- DF.TRU2[131, 6]
...
Download e descompressão dos microdados de estabelecimentos da RAIS 2018.

```{r}
url <- "ftp://ftp.mtps.gov.br/pdet/microdados/RAIS/2018/RAIS_ESTAB_PUB.7z"

download.file(url, "data-raw/rais.7z", mode="wb")
archive_extract("data-raw/rais.7z", dir="data-raw/")
rais <- fread("data-raw/RAIS_ESTAB_PUB.txt")
```

Cômputo da participação dos vínculos em hotéis (CNAE 55108) no total de vínculos dos estabelecimentos de alojamento (inclui o CNAE 55906).

```{r}
rais <- rais[(`CNAE 2.0 Classe`==55108 | `CNAE 2.0 Classe`==55906), c(8, 18)]
names(rais) <- c("vinculos", "cnae")
rais <- rais[, .(sum(vinculos)), by=.(cnae)]
participacao.hoteis.alojamento <- rais[cnae==5510801, V1]/colSums(rais)[2]
rm(rais)
gc()
```
...
Antes de ser importado para o ambiente de trabalho, o arquivo de texto com os microdados da POF 2017-2018 precisa ser lido e transformado para um formato de base de dados. Esse procedimento demanda um processamento maior do que outras partes deste código e pode demorar vários minutos, a depender do computador utilizado. Para facilitar, o resultado da leitura é salvo no formato RDS (nativo do R) para utilização posterior.
...
Tabela3 <- 
  data.table(`Categoria de produtos`=P.nomes,
             ...
             `Participação da oferta turística na oferta total (OT/O)`= 
               DTT.i / DT.i)[`Oferta turística total (OT)`>0] %>%
  adorn_totals("row") %>% 
